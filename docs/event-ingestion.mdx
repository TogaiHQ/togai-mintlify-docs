...
In the previous section, we went through [how to define the data structure](/docs/event-schemas) to send usage data to Togai. In this section, we will take you through how to start sending the raw usage data, monitor them and debug if required.
...
* **accountID:** This is the customer's account ID or account alias ID to which the event needs to be associated and metered.
